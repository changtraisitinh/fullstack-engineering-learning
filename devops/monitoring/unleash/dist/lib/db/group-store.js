...
class GroupStore {
    ...
    async getProjectGroups(projectId) {
        const rows = await this.db
            ...
            .whereIn('r.type', util_1.PROJECT_ROLE_TYPES)
    ...
    async getOldGroupsForExternalUser(userId, externalGroups) {
        const rows = await this.db(`${T.GROUP_USER} as gu`)
            ...
            .where({ 'gu.user_id': userId, 'gu.created_by': exports.SSO_SYNC_USER });
        return rows.map(rowToGroupUser);
    }
