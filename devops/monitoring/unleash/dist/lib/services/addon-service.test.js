...
function getSetup() {
    ...
    const eventService = (0, internals_1.createFakeEventsService)(config);
    const tagTypeService = new tag_type_service_1.default(stores, { getLogger: no_logger_1.default }, eventService);
    const integrationEventsService = new internals_1.IntegrationEventsService(stores, {
...
test('should store ADDON_CONFIG_UPDATE event', async () => {
    ...
    const addonConfig = await addonService.createAddon(config, types_1.TEST_AUDIT_USER);
    const updated = { ...addonConfig, description: 'test' };
    await addonService.updateAddon(addonConfig.id, updated, types_1.TEST_AUDIT_USER);
    const { events } = await eventService.getEvents();
    expect(events.length).toBe(3);
...
test('should store ADDON_CONFIG_REMOVE event', async () => {
    ...
    const addonConfig = await addonService.createAddon(config, types_1.TEST_AUDIT_USER);
    await addonService.removeAddon(addonConfig.id, types_1.TEST_AUDIT_USER);
    const { events } = await eventService.getEvents();
    expect(events.length).toBe(3);
...
test('should hide sensitive fields when fetching', async () => {
    ...
    const createdConfig = await addonService.createAddon(config, types_1.TEST_AUDIT_USER);
    const addons = await addonService.getAddons();
...
test('should reject addon config with missing required parameter when creating', async () => {
    ...
    await expect(async () => addonService.createAddon(config, types_1.TEST_AUDIT_USER)).rejects.toThrow(joi_1.ValidationError);
});
test('should reject updating addon config with missing required parameter', async () => {
    ...
    const config = await addonService.createAddon(addonConfig, types_1.TEST_AUDIT_USER);
    const updated = {
        ...config,
        parameters: { var: 'some-new-value' },
        description: 'test',
    };
    await expect(async () => addonService.updateAddon(config.id, updated, types_1.TEST_AUDIT_USER)).rejects.toThrow(joi_1.ValidationError);
});
