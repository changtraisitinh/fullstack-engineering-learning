...
const feature_event_formatter_md_events_1 = require("./feature-event-formatter-md-events");
var LinkStyle;
(function (LinkStyle) {
...
class FeatureEventFormatterMd {
    ...
    generateFeatureStrategyChangeText(event) {
        ...
        if (type === types_1.FEATURE_STRATEGY_UPDATE && (data || preData)) {
            const strategyText = () => {
    ...
    userWithIdStrategyChangeText(event) {
    ...
    format(event) {
        ...
        const { label, action, path } = feature_event_formatter_md_events_1.EVENT_MAP[type] || {
            label: type,
        ...
        const text = mustache_1.default.render(action, context);
        const url = path
            ? `${this.unleashUrl}${mustache_1.default.render(path, context)}`
            : undefined;
        return {
            label,
            text,
