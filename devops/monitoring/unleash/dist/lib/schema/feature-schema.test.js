...
test("shouldn't allow . nor .. as name", () => {
    ...
    expect(feature_schema_1.featureSchema.validate(toggle1).error?.details[0].message).toEqual('"name" must be URL friendly');
    expect(feature_schema_1.featureSchema.validate(toggle2).error?.details[0].message).toEqual('"name" must be URL friendly');
});
