...
describe('isOSS', () => {
    test('Config with pro environment should set isOss to false regardless of pro casing', async () => {
        const isOss = (0, create_config_1.resolveIsOss)(false, false, 'Pro');
        expect(isOss).toBe(false);
        const lowerCase = (0, create_config_1.resolveIsOss)(false, false, 'pro');
        expect(lowerCase).toBe(false);
        const strangeCase = (0, create_config_1.resolveIsOss)(false, false, 'PrO');
        expect(strangeCase).toBe(false);
    });
    test('Config with enterpriseVersion set should set isOss to false', async () => {
        const isOss = (0, create_config_1.resolveIsOss)(true, false, 'Enterprise');
        expect(isOss).toBe(false);
    });
    test('Config with no enterprise version and any other environment than pro should have isOss as true', async () => {
        const isOss = (0, create_config_1.resolveIsOss)(false, false, 'my oss environment');
    ...
    test('Config with enterprise false and isOss option set to false should return false in test mode', async () => {
        const isOss = (0, create_config_1.resolveIsOss)(false, false, 'my environment', true);
        expect(isOss).toBe(false);
    });
    test('Config with isOss option set to true should return true when test environment is active', async () => {
        let isOss = (0, create_config_1.resolveIsOss)(false, true, 'Pro', true);
        expect(isOss).toBe(true);
        isOss = (0, create_config_1.resolveIsOss)(true, true, 'Pro', true);
        expect(isOss).toBe(true);
        isOss = (0, create_config_1.resolveIsOss)(false, true, 'some environment', true);
        expect(isOss).toBe(true);
    });
});
