...
class FeatureToggleService {
    ...
    async deleteFeatures(featureNames, projectId, auditUser) {
        ...
        await this.eventService.storeEvents(eligibleFeatures.map((feature) => new types_1.FeatureDeletedEvent({
            featureName: feature.name,
            auditUser,
            project: feature.project,
            preData: feature,
    ...
    async reviveFeatures(featureNames, projectId, auditUser) {
        ...
        await this.eventService.storeEvents(eligibleFeatures.map((feature) => new types_1.FeatureRevivedEvent({
            featureName: feature.name,
            auditUser,
            project: feature.project,
        })));
    ...
    async reviveFeature(featureName, auditUser) {
        const feature = await this.featureToggleStore.get(featureName);
        if (!feature) {
            throw new notfound_error_1.default(`Feature ${featureName} does not exist`);
        }
        ...
        await this.eventService.storeEvent(new types_1.FeatureRevivedEvent({
            auditUser,
            featureName,
            project: toggle.project,
        }));
    }
    async getAllArchivedFeatures(archived, userId) {
    ...
    async getArchivedFeaturesByProjectId(archived, project) {
    ...
    async updateVariantsOnEnv(featureName, project, environment, newVariants, user, auditUser) {
        ...
        const { newDocument } = await (0, fast_json_patch_1.applyPatch)((0, fast_json_patch_1.deepClone)(oldVariants), newVariants);
    ...
    async saveVariants(featureName, project, newVariants, auditUser) {
        ...
        await this.eventService.storeEvent(new types_1.FeatureVariantEvent({
            project,
            featureName,
            auditUser,
            oldVariants,
    ...
    async verifyLegacyVariants(featureName) {
        ...
        if (!useLegacyVariants) {
            throw new invalid_operation_error_1.default(`Environment variants deprecated for feature: ${featureName}. Use strategy variants instead.`);
        }
    }
    ...
    async legacySaveVariantsOnEnv(projectId, featureName, environment, newVariants, auditUser, oldVariants) {
        ...
        const theOldVariants = oldVariants ||
            ...
            [];
        await this.eventService.storeEvent(new types_1.EnvironmentVariantEvent({
            featureName,
            environment,
            project: projectId,
    ...
    async setVariantsOnEnvs(projectId, featureName, environments, newVariants, auditUser) {
        ...
        await this.eventService.storeEvents(environments.map((environment) => new types_1.EnvironmentVariantEvent({
            featureName,
            environment,
            project: projectId,
    ...
    async updatePotentiallyStaleFeatures() {
        ...
        if (potentiallyStaleFeatures.length > 0) {
            return this.eventService.storeEvents(potentiallyStaleFeatures
                ...
                .map(({ name, project }) => new types_1.PotentiallyStaleOnEvent({
                featureName: name,
    ...
    async setFeatureCreatedByUserIdFromEvents() {
        ...
        if (updated !== undefined) {
            this.eventBus.emit(metric_events_1.FEATURES_CREATED_BY_PROCESSED, {
                updated,
            });
        }
    }
