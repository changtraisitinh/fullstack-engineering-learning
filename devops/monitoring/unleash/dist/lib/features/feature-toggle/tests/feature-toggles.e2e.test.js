...
test('Should not allow changing project to target project without the same enabled environments', async () => {
    ...
    //@ts-ignore
    user, 'default', types_1.TEST_AUDIT_USER)).rejects.toThrow(new incompatible_project_error_1.default(targetProject));
});
test('Should allow changing project to target project with the same enabled environments', async () => {
    ...
    await expect(async () => app.services.projectService.changeProject(targetProject, featureName, 
    //@ts-ignore
    user, 'default', types_1.TEST_AUDIT_USER)).resolves;
});
...
test('should change strategy sort order when payload is valid', async () => {
    const flag = { name: (0, uuid_1.v4)(), impressionData: false };
    await app.request
        .post('/api/admin/projects/default/features')
        .send({
        name: flag.name,
    ...
    const { body: strategyTwo } = await app.request
        ...
        .expect(200);
    const { body: strategies } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/default/strategies`);
    expect(strategies[0].sortOrder).toBe(0);
    ...
    await app.request
        ...
        .expect(200);
    const { body: strategiesOrdered } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/default/strategies`);
...
test('should return strategies in correct order when new strategies are added', async () => {
    ...
    await app.request
        ...
        .expect(200);
    const { body: strategiesReOrdered } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/default/strategies`);
...
test('should disable strategies in place', async () => {
    ...
    const { body: updatedStrategyOne } = await app.request
        ...
        .expect(200);
    const { body: updatedStrategies } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/default/strategies`);
