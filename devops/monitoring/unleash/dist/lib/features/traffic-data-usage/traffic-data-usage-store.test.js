...
beforeAll(async () => {
    db = await (0, database_init_1.default)('traffic_data_usage_serial', no_logger_1.default, {
        experimental: {
            flags: {},
        },
    });
...
test('can query for monthly aggregation of data for a specified range', async () => {
    ...
    for (let i = 0; i <= 12; i++) {
        const then = (0, date_fns_1.subMonths)(now, i);
    ...
    for (const monthsBack of [3, 6, 12]) {
        ...
        const from = (0, date_fns_1.subMonths)((0, date_fns_1.startOfMonth)(now), monthsBack);
        ...
        for (const entry of result) {
            const index = (0, date_fns_1.differenceInCalendarMonths)(now, new Date(entry.month));
