...
const createApiTokenService_1 = require("../api-tokens/createApiTokenService");
let stores;
let db;
let projectService;
let accessService;
...
beforeAll(async () => {
    ...
    eventService = (0, index_1.createEventsService)(db.rawDatabase, config);
    accessService = (0, index_1.createAccessService)(db.rawDatabase, config);
    featureToggleService = (0, index_1.createFeatureToggleService)(db.rawDatabase, config);
    environmentService = new environment_service_1.default(stores, config, eventService);
    projectService = (0, index_1.createProjectService)(db.rawDatabase, config);
...
test('should update project', async () => {
    ...
    await projectService.updateProject(updatedProject, types_1.TEST_AUDIT_USER);
...
test('should archive project', async () => {
    ...
    await projectService.createProject(project, user, types_1.TEST_AUDIT_USER);
    await projectService.archiveProject(project.id, types_1.TEST_AUDIT_USER);
    const events = await stores.eventStore.getEvents();
    expect(events[0]).toMatchObject({
        type: 'project-archived',
        createdBy: types_1.TEST_AUDIT_USER.username,
    });
    const projects = await projectService.getProjects();
...
test('should revive project', async () => {
    ...
    await projectService.createProject(project, user, types_1.TEST_AUDIT_USER);
    await projectService.archiveProject(project.id, types_1.TEST_AUDIT_USER);
    await projectService.reviveProject(project.id, types_1.TEST_AUDIT_USER);
    const events = await stores.eventStore.getEvents();
    expect(events[0]).toMatchObject({
...
describe('Managing Project access', () => {
    ...
    test('Admin group members should be allowed to add any project role', async () => {
        ...
        await accessService.setUserRootRole(viewerUser.id, model_1.RoleName.VIEWER);
        const adminRole = await stores.roleStore.getRoleByName(model_1.RoleName.ADMIN);
        ...
        [], // groups
        [opsUser.id], // users
        (0, util_1.extractAuditInfoFromUser)(viewerUser))).resolves.not.toThrow();
    });
    ...
    test('Users with project role should only be allowed to grant same role to others', async () => {
        ...
        await expect(async () => projectService.addAccess(project.id, [ownerRole.id], [], [secondUser.id], projectAuditUser)).rejects.toThrow(new error_1.InvalidOperationError('User tried to grant role they did not have access to'));
    });
    test('Users that are members of a group with project role should only be allowed to grant same role to others', async () => {
        ...
        await expect(projectService.addAccess(project.id, [ownerRole.id], [], [secondUser.id], projectAuditUser)).rejects.toThrow(new error_1.InvalidOperationError('User tried to grant role they did not have access to'));
    });
...
test('should return 404 if no active project is found with the project id', async () => {
    ...
    await projectService.archiveProject(newProject.id, types_1.TEST_AUDIT_USER);
    try {
        await projectService.changeProject('newProject', flag.name, user, project.id, auditUser);
    }
    catch (err) {
...
test('should change project when checks pass', async () => {
    ...
    const flag = { name: (0, random_id_1.randomId)() };
...
test('should require equal project environments to move features', async () => {
    ...
    await expect(() => projectService.changeProject(projectB.id, flag.name, user, projectA.id, auditUser)).rejects.toThrowError(incompatible_project_error_1.default);
});
...
test('Should bulk update of only users', async () => {
    ...
    const auditUserFromOps = (0, util_1.extractAuditInfoFromUser)(opsUser);
...
test('should calculate average time to production', async () => {
    ...
    await Promise.all(featureFlags.map((flag) => updateFeature(flag.name, { created_at: (0, date_fns_1.subDays)(new Date(), 15) })));
...
test('should calculate average time to production ignoring some items', async () => {
    ...
    await environmentService.addEnvironmentToProject('customEnv', project.id, types_1.SYSTEM_USER_AUDIT);
    ...
    await eventService.storeEvent(new events_1.FeatureEnvironmentEvent(makeEvent(flag.name)));
    // ignore events added after first enabled
    await updateEventCreatedAt((0, date_fns_1.addDays)(new Date(), 1), flag.name);
    await eventService.storeEvent(new events_1.FeatureEnvironmentEvent(makeEvent(flag.name)));
    ...
    await eventService.storeEvent(new events_1.FeatureEnvironmentEvent(makeEvent(otherProjectFlag.name)));
    ...
    await eventService.storeEvent(new events_1.FeatureEnvironmentEvent(makeEvent(nonReleaseFlag.name)));
    ...
    await eventService.storeEvent(new events_1.FeatureEnvironmentEvent(makeEvent(previouslyDeleteFlag.name)));
    await updateEventCreatedAt((0, date_fns_1.subDays)(new Date(), 30), previouslyDeleteFlag.name);
    const result = await projectService.getStatusUpdates(project.id);
...
test('should get correct amount of features created in current and past window', async () => {
    ...
    await Promise.all([
        updateFeature(flags[2].name, { created_at: (0, date_fns_1.subDays)(new Date(), 31) }),
        updateFeature(flags[3].name, { created_at: (0, date_fns_1.subDays)(new Date(), 31) }),
    ]);
    const result = await projectService.getStatusUpdates(project.id);
...
test('should return average time to production per flag', async () => {
    ...
    await Promise.all(flags.map((flag) => updateFeature(flag.name, {
        created_at: (0, date_fns_1.subDays)(new Date(), flag.subdays),
    })));
...
test('should return average time to production per flag for a specific project', async () => {
    ...
    const featureFlagsProject1 = await Promise.all(flagsProject1.map((flag) => {
        return featureToggleService.createFeatureToggle(project1.id, flag, auditUser);
    }));
    const featureFlagsProject2 = await Promise.all(flagsProject2.map((flag) => {
        return featureToggleService.createFeatureToggle(project2.id, flag, auditUser);
    }));
    await Promise.all(featureFlagsProject1.map((flag) => {
        return eventService.storeEvent(new events_1.FeatureEnvironmentEvent({
            enabled: true,
            project: project1.id,
            featureName: flag.name,
    ...
    await Promise.all(featureFlagsProject2.map((flag) => {
        return eventService.storeEvent(new events_1.FeatureEnvironmentEvent({
            enabled: true,
            project: project2.id,
            featureName: flag.name,
    ...
    await Promise.all(flagsProject1.map((flag) => updateFeature(flag.name, {
        created_at: (0, date_fns_1.subDays)(new Date(), flag.subdays),
    })));
    await Promise.all(flagsProject2.map((flag) => updateFeature(flag.name, {
        created_at: (0, date_fns_1.subDays)(new Date(), flag.subdays),
    })));
...
describe('feature flag naming patterns', () => {
    test(`should clear existing example and description if the payload doesn't contain them`, async () => {
        ...
        await projectService.createProject(project, user, auditUser);
        await projectService.updateProjectEnterpriseSettings(project, (0, util_1.extractAuditInfoFromUser)(user));
        ...
        }, (0, util_1.extractAuditInfoFromUser)(user));
...
test('should get project settings with mode', async () => {
    ...
    await projectService.updateProjectEnterpriseSettings({ mode, id }, (0, util_1.extractAuditInfoFromUser)(user));
    const projects = await projectService.getProjects();
...
describe('create project with environments', () => {
    ...
    test("envs that don't exist cause errors", async () => {
        await expect(createProjectWithEnvs(['fake-project'])).rejects.toThrow(error_1.BadDataError);
...
describe('automatic ID generation for create project', () => {
    ...
    test.each(['', undefined, '     '])('An id with the value `%s` is treated as missing (and the id is based on the name)', async (id) => {
        const name = (0, random_id_1.randomId)();
    ...
    describe('backwards compatibility', () => {
        ...
        test.each([true, false])('if the ID is present in the input, it is used as the ID regardless of the feature flag states. Flag state: %s', async (flagState) => {
            const id = (0, random_id_1.randomId)();
