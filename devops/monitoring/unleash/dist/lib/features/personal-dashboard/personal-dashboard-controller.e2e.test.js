...
let db;
beforeAll(async () => {
    db = await (0, database_init_1.default)('personal_dashboard', no_logger_1.default);
...
test('should return projects where users are part of a group', async () => {
    const { body: user1 } = await loginUser('user1@test.com');
    const projectA = await createProject(`x${(0, util_1.randomId)()}`, user1);
    const { body: user2 } = await loginUser('user2@test.com');
...
test('should return personal dashboard project details', async () => {
    ...
    const project = await createProject(`x${(0, util_1.randomId)()}`, user1);
    const { body: user2 } = await loginUser('user2@test.com');
...
test("should return 404 if the project doesn't exist", async () => {
    ...
    await app.request
        .get(`/api/admin/personal-dashboard/${(0, util_1.randomId)()}`)
        .expect(404);
});
