...
const createFlag = async (flag, strategy = defaultStrategy, tags = [], projectId = 'default', username = 'test', userId = -9999) => {
    await app.services.featureToggleService.createFeatureToggle(projectId, flag, types_1.TEST_AUDIT_USER);
    if (strategy) {
...
const createVariants = async (feature, variants) => {
    await app.services.featureToggleService.legacySaveVariantsOnEnv(types_1.DEFAULT_PROJECT, feature, util_1.DEFAULT_ENV, variants, types_1.TEST_AUDIT_USER);
};
const createProjects = async (projects = [types_1.DEFAULT_PROJECT], featureLimit = 2) => {
...
const createSegment = (postData) => {
    return app.services.segmentService.create(postData, types_1.TEST_AUDIT_USER);
};
...
test('import too many feature exceeding limit', async () => {
    ...
    await createProjects([types_1.DEFAULT_PROJECT], featureLimit);
...
test('should not import archived parent', async () => {
    ...
    const { body } = await app.getProjectFeatures(types_1.DEFAULT_PROJECT);
...
test(`should give errors with flag names if the flags don't match the project pattern`, async () => {
    ...
    for (const project of [types_1.DEFAULT_PROJECT]) {
...
test('should import features from file', async () => {
    ...
    await app.linkProjectToEnvironment(types_1.DEFAULT_PROJECT, util_1.DEFAULT_ENV);
    await app.services.importService.importFromFile('src/lib/features/export-import-toggles/import-data.json', types_1.DEFAULT_PROJECT, util_1.DEFAULT_ENV);
    const { body: importedFeature } = await getFeature(defaultFeatureName);
    expect(importedFeature).toMatchObject({
        name: defaultFeatureName,
        project: types_1.DEFAULT_PROJECT,
