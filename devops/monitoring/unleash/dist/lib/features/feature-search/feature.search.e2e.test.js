...
let stores;
beforeAll(async () => {
    db = await (0, database_init_1.default)('feature_search', no_logger_1.default, {
        dbInitMethod: 'legacy',
    });
    ...
    await stores.accessStore.addPermissionsToRole(body.rootRole, [
        { name: types_1.UPDATE_FEATURE_ENVIRONMENT },
        { name: types_1.CREATE_FEATURE_STRATEGY },
    ], 'development');
    await stores.environmentStore.create({
        name: 'production',
        type: 'production',
...
test('should sort features', async () => {
    ...
    await (0, test_helper_1.insertLastSeenAt)('my_feature_c', db.rawDatabase, 'default');
...
test('should return segments in payload with no duplicates/nulls', async () => {
    ...
    }, util_1.DEFAULT_ENV, 'my_feature_a');
...
test('should filter features by segment', async () => {
    ...
    }, util_1.DEFAULT_ENV, 'my_feature_c');
    await app.createFeature('my_feature_d');
    await app.addStrategyToFeatureEnv({
        name: 'default',
        segments: [mySegmentC.id],
    }, util_1.DEFAULT_ENV, 'my_feature_d');
    await app.addStrategyToFeatureEnv({
        name: 'default',
        segments: [mySegmentA.id],
    }, util_1.DEFAULT_ENV, 'my_feature_d');
