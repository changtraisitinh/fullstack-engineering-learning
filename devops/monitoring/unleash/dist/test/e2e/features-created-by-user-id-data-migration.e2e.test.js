...
beforeAll(async () => {
    ...
    db = await (0, database_init_1.default)('features_created_by_user_id_migration', config.getLogger);
    unleashConfig = config;
    stores = db.stores;
    service = (0, features_1.createFeatureToggleService)(db.rawDatabase, config);
...
test('admin tokens get populated to admin token user', async () => {
    ...
    const test2 = features.filter((f) => f.created_by_user_id === types_1.ADMIN_TOKEN_USER.id);
...
test('emits event with updated rows count', async () => {
    ...
    let triggered = false;
    eventBus.on(metric_events_1.FEATURES_CREATED_BY_PROCESSED, ({ updated }) => {
