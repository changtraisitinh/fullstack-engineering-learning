...
test('Anonymises name, username and email fields if anonymiseEventLog flag is set', async () => {
    const anonymisedApp = await (0, test_helper_1.setupAppWithCustomConfig)(stores, { experimental: { flags: { anonymiseEventLog: true } } }, db.rawDatabase);
...
test('creates user with email md5 hash', async () => {
    ...
    const user = await db
        ...
        .first(['email_hash']);
    const expectedHash = (0, crypto_1.createHash)('md5')
