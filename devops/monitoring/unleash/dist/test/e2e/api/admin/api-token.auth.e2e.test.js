...
test('editor users should only get client or frontend tokens', async () => {
    ...
    const preHook = (app, config, { userService, accessService }) => {
        app.use('/api/admin/', async (req, res, next) => {
            const role = await accessService.getPredefinedRole(model_1.RoleName.EDITOR);
    ...
    const { request, destroy } = await (0, test_helper_1.setupAppWithCustomAuth)(stores, preHook, undefined, db.rawDatabase);
...
test('viewer users should not be allowed to fetch tokens', async () => {
    ...
    const preHook = (app, config, { userService, accessService }) => {
        app.use('/api/admin/', async (req, res, next) => {
            const role = await accessService.getPredefinedRole(model_1.RoleName.VIEWER);
            const user = await userService.createUser({
...
test('Token-admin should not be allowed to create token', async () => {
    ...
    const preHook = (app, config, { userService, accessService }) => {
        app.use('/api/admin/', async (req, res, next) => {
            const role = await accessService.getPredefinedRole(model_1.RoleName.ADMIN);
            req.user = await userService.createUser({
...
test('An admin should be forbidden to create an admin token', async () => {
    const { request, destroy, services } = await (0, test_helper_1.setupAppWithAuth)(stores, undefined, db.rawDatabase);
...
test('A role with only CREATE_PROJECT_API_TOKEN can create project tokens', async () => {
    ...
    const preHook = (app, config, { userService, accessService, }) => {
        app.use('/api/admin/', async (req, res, next) => {
            ...
            const createClientApiTokenRole = await accessService.createRole({
                ...
                permissions: [{ name: types_1.CREATE_PROJECT_API_TOKEN }],
                type: 'root-custom',
    ...
    const { request, destroy } = await (0, test_helper_1.setupAppWithCustomAuth)(stores, preHook, {}, db.rawDatabase);
    await request
...
describe('Fine grained API token permissions', () => {
    describe('A role with access to CREATE_CLIENT_API_TOKEN', () => {
        test('should be allowed to create client tokens', async () => {
            const preHook = (app, config, { userService, accessService, }) => {
                app.use('/api/admin/', async (req, res, next) => {
                    const builtInRole = await accessService.getPredefinedRole(model_1.RoleName.VIEWER);
                    const user = await userService.createUser({
                    ...
                    await accessService.addPermissionToRole(builtInRole.id, types_1.CREATE_CLIENT_API_TOKEN);
                    await accessService.addUserToRole(user.id, createClientApiTokenRole.id, 'default');
                    next();
                });
            };
