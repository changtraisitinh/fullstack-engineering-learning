...
test('should serve the OpenAPI spec with a `version` property', async () => {
    return app.request
        ...
        expect(version).toMatch(new RegExp(`^${semver_1.default.clean(version) ?? 'invalid semver'}`));
...
describe('subpath handling', () => {
    ...
    beforeAll(async () => {
        appWithSubPath = await (0, test_helper_1.setupAppWithBaseUrl)(db.stores, subPath);
    });
    afterAll(async () => {
